...
class ConfusionMatrix(ConfusionMatrix):
    ...
    def __init__(self, num_classes):
        super().__init__(num_classes, normalize=None)
    ...
    def update(self, preds, target):
        ...
        if target.dim() == 2 and target.shape[1] == self.num_classes:
            confmat = scatter_add(target.float(), preds, dim=0, dim_size=self.num_classes)
...
def save_confusion_matrix(cm, path2save, ordered_names):
    ...
    plt.savefig(path_precision, format="svg", tight_layout=True, bbox_inches="tight")
