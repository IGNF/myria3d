...
Run the following for help on the parameters:
...
class FrenchLidarDataPreparation:
    rgb_names = ["red", "green", "blue"]
    standard_features_names = (
        ["intensity", "return_num", "num_returns"] + rgb_names + ["nir"]
    )
    additional_features_names = ["composite"]
    ...
    def load_las(self, las_filepath: str):
        ...
        x = np.asarray(
            [las[x_name] for x_name in self.standard_features_names],
            dtype=np.float32,
        ).transpose()
        if "composite" in self.additional_features_names:
            composite = (
                np.asarray([las[x_name] for x_name in self.rgb_names], dtype=np.float32)
                .transpose()
                .mean(axis=1, keepdims=True)
            )
            x = np.concatenate([x, composite], axis=1)
        ...
        return Data(
            ...
            x_features_names=self.standard_features_names
